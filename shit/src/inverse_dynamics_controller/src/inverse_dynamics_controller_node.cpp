...
public:
  InverseDynamicsControllerNode()
    ...
    P_a_ = declare_parameter<double>("P_a", 280.0);
    D_a_ = declare_parameter<double>("D_a", 26.7);
    P_h_ = declare_parameter<double>("P_h", 80.0);
    D_h_ = declare_parameter<double>("D_h", 20.0);
    P_l_ = declare_parameter<double>("P_l", 80.0);
    D_l_ = declare_parameter<double>("D_l", 20.0);
    P_vh_ = declare_parameter<double>("P_vh", 40.0);
    P_vl_ = declare_parameter<double>("P_vl", 30.0);
    P_F_ = declare_parameter<double>("P_F", 2.0);
    I_F_ = declare_parameter<double>("I_F", 10.0);
    D_F_ = declare_parameter<double>("D_F", 8.0);
    F_touch_ = declare_parameter<double>("F_touch", 0.10);
    F_lose_ = declare_parameter<double>("F_lose", 0.05);
    force_integral_limit_ =
        eclare_parameter<double>("force_integral_limit", 10.0);
    tau_a_max_ = declare_parameter<double>("tau_a_max", 1000.0);
    tau_h_max_ = declare_parameter<double>("tau_h_max", 20.0);
    tau_l_max_ = declare_parameter<double>("tau_l_max", 1000.0);
    q1_dd_max_ = declare_parameter<double>("q1_dd_max", 100.0);
    q2_dd_max_ = declare_parameter<double>("q2_dd_max", 50.0);
    q3_dd_max_ = declare_parameter<double>("q3_dd_max", 50.0);
...
private:
  ...
  double computeAlphaAcc(double a, double ad) const {
  ...
  double computeHAcc(double h, double hd, double F_meas, double dt) {
    ...
    case bow_interfaces::msg::BowTarget::MODE_FORCE: {
      ...
      force_integral_ =
          clamp(force_integral_, -force_integral_limit_, force_integral_limit_);
  ...
  double computeLAcc(double l, double ld) const {
  ...
  void jointStateCallback(const sensor_msgs::msg::JointState::SharedPtr msg) {
    ...
    const double q1_dd = computeAlphaAcc(a, ad);
    const double q2_dd = computeHAcc(h, hd, F_meas, dt);
    const double q3_dd = computeLAcc(l, ld);
